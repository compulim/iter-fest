...
describe.each([[[1, 2, 3]], [[2]], [[]]])('when compare to %s.findLastIndex()', (array: number[]) => {
  ...
  beforeEach(() => {
    ...
    iterableResult = iterableFindLastIndex(iterable, iterablePredicate, thisArg);
  });
  ...
  if (array.length === 3) {
    ...
    describe('the predicate', () => {
      test('should have called the 1 times', () => expect(iterablePredicate).toHaveBeenCalledTimes(1));
      test('should have called in similar way', () => {
        expect(iterablePredicate).toHaveBeenNthCalledWith(1, 2, 0, iterable);
        expect(iterablePredicate).toHaveNthReturnedWith(1, 0);
        expect(iterablePredicate.mock.contexts[0]).toBe(thisArg);
      });
    });
  } else {
...
test('should throw TypeError when passing an invalid callbackFn', () =>
  ...
  expect(() => iterableFindLastIndex([], 0 as any)).toThrow('is not a function'));
