...
describe.each([[[1, 2, 3]], [[]]])('when compare to %s.find()', array => {
  ...
  beforeEach(() => {
    ...
    arrayResult = array.find(arrayPredicate, thisArg);
    iterableResult = iterableFind(iterable, iterablePredicate, thisArg);
  });
...
test('should throw TypeError when passing an invalid callbackFn', () =>
  ...
  expect(() => iterableFind([], 0 as any)).toThrow('is not a function'));
